...
const list = async (req, res, next) => {
    ...
    try {
        const pedidos = await Promise.all(await PedidoModel.findAll({
            where: {
        ...
        const response = await Promise.all(await pedidos.map(async pedido => (
            {
                pedido: pedido,
...
async function getProdutos(idCarrinho) {
    ...
    return Promise.all(await itens
        .map(async item => {
