<template>
    <v-main class="main">
        <v-card>
            <v-card-title>
                Pedido #{{compra.pedido.id}}
            </v-card-title>
            <v-card-subtitle>
                Status : {{status}}
            </v-card-subtitle>
            <v-card-title>
                <label> Endereço : {{ compra.endereco.rua }} nº {{ compra.endereco.numero }} </label>
            </v-card-title>
            <v-card-subtitle>
            ...
            <div class="snk-card">
                <div class="snk-flex-col" v-for="(card, idx) in compra.produtos" :key="idx">
                    <v-img
                            :src="card.urlImage"
                            width="200px"
                            height="200px"
    ...
    export default {
        ...
        methods: {
            buscaPedidos() {
                ...
                return _buscaPedidos(acquisitionsId);
            },
        },
        async created() {
